...
char ** parse_message(const char * msg) {
  ...
  char * message = (char*) malloc( sizeof(msg) * sizeof(char) );
...
//  char * header = strdup(token2);
//  char * value = strdup(token3);
//  
  

  
  char **messageOutput = (char**) malloc( sizeof(keyword) + sizeof(header) + sizeof(value) );

  messageOutput[0] = (char*) malloc( sizeof(keyword) );
  messageOutput[1] = (char*) malloc( sizeof(header)  );
  messageOutput[2] = (char*) malloc( sizeof(value)   );

  strcpy( messageOutput[0], keyword );
  strcpy( messageOutput[1], header  );
  strcpy( messageOutput[2], value   );


  Serial.println( messageOutput[0] );
  Serial.println( messageOutput[1] );
  Serial.println( messageOutput[2] );

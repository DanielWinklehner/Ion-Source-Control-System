...
def round_off_mist1_notation(number, precision):
    ...
    if (number[-1] == '-') and (number[-2] == '1'):
        ...
        if precision == 1:
            rounded_off_number = number[0] + number[1] + '0' + number[
                                                               2:precision + 1]  # + 1 for sign of number. Another + 1 would be for the mentissa but since it's going to be 0 which is not written here, we exclude it. So, it becomes just a single +1.
        else:
...
def build_output_message(channels_and_precisions, values):
    ...
    for (i, (channel_name, precision)) in enumerate(channels_and_precisions.items()):
        ...
        output_message += str(channel_name) + str(
            round_off_mist1_notation(str(convert_scientific_notation_to_mist1(float(values[i]), int(precision))),
                                     int(precision)))

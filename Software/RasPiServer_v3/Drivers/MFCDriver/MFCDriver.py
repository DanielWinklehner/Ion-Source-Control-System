...
class MFCDriver:
    ...
    @staticmethod
    def translate_gui_to_device(server_to_driver):
        ...
        for i in range(0, num_of_mesg):
            drivers_response_to_server.append(
                build_message(msg_type=server_to_driver['set'], device_address=server_to_driver['device_id'],
                              for_what=server_to_driver['channel_ids'][i]))

        return drivers_response_to_server

    @staticmethod
    def translate_device_to_gui(response):
        ...
        for i in range(0, num_of_mesg):
            parsed_message = parse_message(response[i])
            if parsed_message['acknowledged']:
                ...
                drivers_response_to_server.append(parsed_message['value'])

            else:
                drivers_response_to_server.append("Error: " + get_error(parsed_message['value']))
...
if __name__ == '__main__':
    ...
    print(x.translate_device_to_gui(response_from_driver))

...
class SerialCOM:
                ...
                try:
                    ...
                    if "device_id" in response and "=" in response:
                        ...
                        if arduino_id == response.split("=")[1]:
                            print "Found the Arduino corresponding to UUID %s at port %s" % (
                            arduino_id, serial_port_name)
    ...
    def is_alive(self):
        ...
        try:
            ...
            while (response.strip() != "device_id=" + self._arduino_id) and (
                (time.time() - first_message_time) < float(self._alive_timeout)):
                self.send_message("i")
                response = self.read_message()
